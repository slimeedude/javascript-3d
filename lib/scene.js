...
class Camera {
  ...
  worldToCamera(point) {
    ...
    // I had to use YZX order, the camera was turning weird for some reason
...
class Object3D {
  constructor(shape, position = new Point3D(0, 0, 0), rotation = { x: 0, y: 0, z: 0 }, pivot = new Point3D(0, 0, 0)) {
    this.shape = shape;
    this.position = position;
    this.rotation = rotation;
    this.pivot = pivot;
  ...
  getTransformedPoints() {
    return this.shape.points.map(p => {
